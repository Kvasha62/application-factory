...
значения из `tenant_authority.contracts`. Опубликованный модуль и сам не держит
ссылку на транспорт: `import tenant_authority.reader` не связывает ни
`transport`, ни `_CHANNELS`, ни `ASGIContractTransport` — исполнитель контракта
разрешается внутри вызова и не сохраняется в пространстве имён модуля. Через
опубликованную поверхность недоступны хранилище, журнал аудита, таблица идемпотентности, операции мутаций,
