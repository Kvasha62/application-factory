...
- `ADR-0015 — Activate Level 2 Component Factory` — `RATIFIED` (2026-09-14). Активирует Level 2 — Component Factory после прохождения Factory Gate #1, при сохранении существующих бизнес-границ, владения данными, контрактных и compatibility-правил. Реализация фабричных механизмов выполняется инкрементально отдельными implementation slices: Slice A — Component Registry, Slice B — Component Catalog, Slice C — Platform Manifest, Slice D — Golden Bundles и Slice E — Composer реализованы (Slice E — Issue #73, PR открыт, ожидает owner approval); Platform Instance assembly tooling требует отдельного утверждённого work item.
