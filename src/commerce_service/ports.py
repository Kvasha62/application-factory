"""Ports through which Commerce consumes the platform boundaries.

SCS-002 may not read another component's storage, internal modules or
internal types (ARCHITECTURE.md §1.1, §6.1, LAW-04). It depends on narrow
ports — expressed entirely in this component's own vocabulary — and on
nothing else:

* :class:`AuthorizationPort` — the decision of IS-003 for one access
  question. This is why Commerce contains no permission logic, no grant
...
* :class:`CommandSafetyPort` — the exactly-once execution of one
  state-changing command, answered by IS-005. This is why Commerce owns no
  second idempotency mechanism: the create-product command is delivered to
  the IS-005 guard, which decides between executing, replaying and refusing
  a conflict.
* :class:`TenantContextPort` — the effective tenant context of one verified
  ...
  product) whose target does not exist yet, so its authorization question
  ...
  port — no second identity or tenant mechanism is created.
...
with a value of :mod:`commerce_service.consumed`, including when the
...
:mod:`commerce_service.adapters`, which sits over the published consumer
...
from commerce_service.consumed import DecisionOutcome, TenantContextOutcome
...
class TenantContextPort(Protocol):
    ...
    Consumed by exactly one command — create-product — whose target does not
    ...
    client to it. No second identity or tenant mechanism exists here: the
    port carries the answer of IS-001 and nothing else.
    """
...
class CommandSafetyPort(Protocol):
    ...
    command for the same key raises ``IdempotencyConflict``. Commerce
    supplies the command identity of a creation (operation, target,
