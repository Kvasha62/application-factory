...
This is the only place in Commerce that knows the *shape* of another
...
* it reads from the answer only what the published contract documents
  ...
  value of :mod:`commerce_service.consumed`. No provider object outlives
...
import anything of another component, so the rest of Commerce stays free
of it too.
"""
...
class ResourceClaim:
    ...
    client serializes exactly these. This value is owned by Commerce: it is
...
class TenantContextAdapter:
    ...
    handed over by a composition root. The adapter reads from the answer
    only the documented fields (``identity_id``, ``tenant_id``,
    ``platform_id``, ``kind``, ``source``) and converts it immediately into
    a value of :mod:`commerce_service.consumed`; a refusal of the dependency
    — whatever it raised — is a :class:`TenantContextRefusal`, and an answer
    outside the documented shape is one too. Nothing here imports
    ``identity_service``: the adapter is structural over the published
    client, like its sibling :class:`AuthorizationDecisionAdapter`.
    """
