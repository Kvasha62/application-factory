"""Contract transport for a Level 0 modular monolith (SCS-002 Commerce).

ARCHITECTURE.md §1.1 allows exactly four ways for components to interact:
API, events, Data Export/CDC and official integration mechanisms. At Level
0 the component boundary is made real by serving the *published API*
in-process: a request is executed against the component's ASGI application
and the response is returned as plain data.
...
module's private table. So a consumer cannot reach the engine, the stores,
the audit journal or the component's ASGI application through it. Requests
run through the same routing, validation, enforcement-chain and audit code
as network requests would.
"""
...
from commerce_service.errors import ContractViolation
...
class ASGIContractTransport:
    ...
    A dedicated event loop is used only when the caller already runs inside
    a loop; the ordinary synchronous path runs the exchange on the current
    thread.
    """
    ...
    def _ensure_portal_loop(self) -> asyncio.AbstractEventLoop:
        with self._lock:
            if self._portal_loop is None:
                ...
                threading.Thread(
                    target=run, name="commerce-contract-transport", daemon=True
                ).start()
                ready.wait()
    ...
    async def _perform(
        ...
        scope: dict[str, Any] = {
            ...
            "query_string": b"",
            "root_path": "",
            "client": ("commerce_service-consumer", 0),
            "server": ("commerce_service", 80),
            "headers": [
                *(
                    [
        ...
        if status is None:
            raise ContractViolation(
                f"commerce contract produced no response for {method} {path}"
            )
        ...
        try:
            ...
            raise ContractViolation(
                f"commerce contract returned a non-JSON payload for {method} {path}"
            ) from exc
        if not isinstance(payload, Mapping):
            raise ContractViolation(
                f"commerce contract returned a non-object payload for {method} {path}"
            )
        return status, payload


#: Provider-side channel table: handle -> live transport.
...
def open_channel(app: Any) -> str:
    ...
    handle = f"cmr-{secrets.token_urlsafe(16)}"
...
def call_contract(
    ...
    if channel is None:
        raise ContractViolation("the commerce contract channel is closed")
