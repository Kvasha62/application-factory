"""Deterministic structural validation of the Golden Bundle document.

The normative schema lives in
``factory/golden_bundle/schema/golden_bundle.schema.json`` and is the
declarative contract of the bundle format.
...
``oneOf`` — exactly the subset used by the bundle schema.
"""
...
SCHEMA_PATH = "factory/golden_bundle/schema/golden_bundle.schema.json"

SchemaDocument = Mapping[str, Any]


def load_schema(root: Path) -> SchemaDocument:
    """Return the normative bundle schema read from ``root``."""
    path = root / SCHEMA_PATH
    if not path.is_file():
        message = f"golden bundle schema not found: {SCHEMA_PATH}"
        raise FileNotFoundError(message)
    document = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(document, Mapping):
        message = f"golden bundle schema is not a JSON object: {SCHEMA_PATH}"
