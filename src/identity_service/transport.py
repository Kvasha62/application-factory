"""Contract transport for a Level 0 modular monolith (IS-001).
...
private table. So a consumer cannot reach the engine, the identity store, the
audit journal, the record operations or the component's
ASGI application through it. Requests run through the same routing, validation,
identity verification, tenant-context derivation and audit code as network
requests would.
"""
...
class ASGIContractTransport:
    ...
    async def _perform(
        ...
        scope: dict[str, Any] = {
            ...
            "query_string": b"",
            "root_path": "",
            "client": ("identity_service-consumer", 0),
        ...
        if status is None:
            raise ContractViolation(
                f"identity contract produced no response for {method} {path}"
            )
        ...
        try:
            ...
            raise ContractViolation(
                f"identity contract returned a non-JSON payload for {method} {path}"
            ) from exc
        if not isinstance(payload, Mapping):
            raise ContractViolation(
                f"identity contract returned a non-object payload for {method} {path}"
            )
...
#: store. ``identity_service.transport`` is an internal module of this component
...
def open_channel(app: Any) -> str:
    ...
    and useless on its own — every request made through it still has to present a
    credential that the application behind it verifies, and every answer it
    produces is audited exactly as for a network request.
    """
    if not callable(app):
        raise TypeError("a contract channel needs the component's ASGI application")
    handle = f"idc-{secrets.token_urlsafe(16)}"
...
def call_contract(
    ...
    if channel is None:
        raise ContractViolation("the identity contract channel is closed")
