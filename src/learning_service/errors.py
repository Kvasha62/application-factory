"""Error surface of the Learning component (SCS-001).
...
* :class:`AccessRefused` — the access did not happen: either IS-003 decided
  ...
  component's own boundary refused (unknown submission, foreign owner,
...
* :class:`ContractViolation` — this component's contract transport failed or
  ...
  of assuming a submission was served;
...
class AccessRefused(Exception):
    """One refused access at the Learning enforcement boundary.
    ...
    and it can never be mistaken for a served submission.
    """
