"""Contract transport for a Level 0 modular monolith (Learning).
...
module's private table. So a consumer cannot reach the engine, the business
...
class ASGIContractTransport:
    ...
    async def _perform(
        ...
        scope: dict[str, Any] = {
            ...
            "query_string": b"",
            "root_path": "",
            "client": ("learning_service-consumer", 0),
        ...
        if status is None:
            raise ContractViolation(
                f"learning contract produced no response for {method} {path}"
            )
        ...
        try:
            ...
            raise ContractViolation(
                f"learning contract returned a non-JSON payload for {method} {path}"
            ) from exc
        if not isinstance(payload, Mapping):
            raise ContractViolation(
                f"learning contract returned a non-object payload for {method} {path}"
            )
...
#: or the store. ``learning_service.transport`` is an internal module of this
#: component, and a consumer component that imported it would break its own
#: import guard.
...
def open_channel(app: Any) -> str:
    ...
    handle = f"lrn-{secrets.token_urlsafe(16)}"
...
def call_contract(
    ...
    if channel is None:
        raise ContractViolation("the learning contract channel is closed")
