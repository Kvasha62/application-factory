"""Contract transport for a Level 0 modular monolith (SCS-003 Booking).
...
from booking_service.errors import ContractViolation
...
class ASGIContractTransport:
    ...
    def _ensure_portal_loop(self) -> asyncio.AbstractEventLoop:
        with self._lock:
            if self._portal_loop is None:
                ...
                threading.Thread(
                    target=run, name="booking-contract-transport", daemon=True
                ).start()
                ready.wait()
    ...
    async def _perform(
        ...
        scope: dict[str, Any] = {
            ...
            "query_string": b"",
            "root_path": "",
            "client": ("booking_service-consumer", 0),
            "server": ("booking_service", 80),
            "headers": [
                *(
                    [
        ...
        if status is None:
            raise ContractViolation(
                f"booking contract produced no response for {method} {path}"
            )
        ...
        try:
            ...
            raise ContractViolation(
                f"booking contract returned a non-JSON payload for {method} {path}"
            ) from exc
        if not isinstance(payload, Mapping):
            raise ContractViolation(
                f"booking contract returned a non-object payload for {method} {path}"
            )
...
def open_channel(app: Any) -> str:
    ...
    handle = f"bkg-{secrets.token_urlsafe(16)}"
...
def call_contract(
    ...
    if channel is None:
        raise ContractViolation("the booking contract channel is closed")
