"""Ports through which Booking consumes the platform boundaries.

SCS-003 may not read another component's storage, internal modules or
...
* :class:`AuthorizationPort` — the decision of IS-003 for one access
  question. This is why Booking contains no permission logic, no grant
...
* :class:`CommandSafetyPort` — the exactly-once execution of one
  state-changing command, answered by IS-005. This is why Booking owns no
  second idempotency mechanism: the create-resource command is delivered to
...
* :class:`TenantContextPort` — the effective tenant context of one verified
  ...
  resource) whose target does not exist yet, so its authorization question
...
with a value of :mod:`booking_service.consumed`, including when the
...
:mod:`booking_service.adapters`, which sits over the published consumer
...
from booking_service.consumed import DecisionOutcome, TenantContextOutcome
...
class TenantContextPort(Protocol):
    ...
    Consumed by exactly one command — create-resource — whose target does not
...
class CommandSafetyPort(Protocol):
    ...
    command for the same key raises ``IdempotencyConflict``. Booking
