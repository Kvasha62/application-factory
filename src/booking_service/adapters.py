...
This is the only place in Booking that knows the *shape* of another
...
* it reads from the answer only what the published contract documents
  ...
  value of :mod:`booking_service.consumed`. No provider object outlives
...
import anything of another component, so the rest of Booking stays free
of it too.
"""
...
class ResourceClaim:
    ...
    client serializes exactly these. This value is owned by Booking: it is
...
class TenantContextAdapter:
    ...
    a value of :mod:`booking_service.consumed`; a refusal of the dependency
