"""Local value types for what IS-004 consumes from the Authorization Boundary.

The Resource Boundary depends on exactly one fact it does not own: *whether
this verified subject may perform this operation on this tenant-scoped
resource* (IS-003). It obtains the answer through the published consumer
surface of IS-003 and stores nothing of it beyond the audit journal, so this
component contains no second authorization mechanism and no tenant-context
mechanism at all: the effective tenant of a request is whatever the decision
carries, and it exists here only for the duration of one access (invariant 5).

Why local types at all: a consumed answer that kept the provider's own classes
would make the provider's internals part of this component's code. The
boundary is therefore expressed in this module's own vocabulary:
...
* :class:`DependencyRefusal` — the dependency did not produce a usable answer;
  ...
  nothing this component can read. Either way the access is denied: a
  dependency that cannot answer is never an implicit yes (invariant 4).

The decision vocabulary below mirrors the published decision model of IS-003
(``components/authorization/contract``). It is a mirror, not an import: no
module of this component imports ``authorization_service``, so the dependency
is on the published contract, and a composition root may equally wire a remote
client, a stub or a future implementation, as long as it answers the
documented contract. An answer outside the mirrored vocabulary is treated as
non-authoritative and fails closed (:mod:`records_service.engine`).
"""
