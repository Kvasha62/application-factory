"""Contract transport for a Level 0 modular monolith (IS-004).
...
request is executed against the component's ASGI application and the response
is returned as plain data.

Values cross the boundary — never live objects. The transport returns
``(status, payload)`` only, and the consumer holds nothing but an opaque
channel handle: the application itself stays on the provider side, in this
module's private table. So a consumer cannot reach the engine, the resource
store, the audit journal or the component's ASGI application through it.
Requests run through the same routing, validation, enforcement-chain and
audit code as network requests would.
"""
...
from records_service.errors import ContractViolation
...
class ASGIContractTransport:
    ...
    async def _perform(
        ...
        scope: dict[str, Any] = {
            ...
            "query_string": b"",
            "root_path": "",
            "client": ("records_service-consumer", 0),
            "server": ("records_service", 80),
            # A JSON body must be announced in the request headers, not only in
            # the scope: the application parses the body exactly as it would
            # over the network, so an in-process request cannot take a
        ...
        async def receive() -> dict[str, Any]:
            ...
            if not consumed:
                consumed = True
                # The whole body is delivered in one message: announcing more of
                # it and then disconnecting would make every request with a
                # body look like a truncated one to the application.
        ...
        if status is None:
            raise ContractViolation(
                f"records contract produced no response for {method} {path}"
            )
        ...
        try:
            ...
            raise ContractViolation(
                f"records contract returned a non-JSON payload for {method} {path}"
            ) from exc
        if not isinstance(payload, Mapping):
            raise ContractViolation(
                f"records contract returned a non-object payload for {method} {path}"
            )
        return status, payload


#: Provider-side channel table: handle -> live transport. A consumer is handed
#: the handle (a string) and never the transport, so no attribute path from
#: the published client leads to the application, the deployment, the engine
#: or the store. ``records_service.transport`` is an internal module of this
#: component (see ``api.consumer_surface.internal_modules`` in the component
#: contract), and a consumer component that imported it would break its own
#: import guard.
...
def open_channel(app: Any) -> str:
    ...
    The handle is a value: unguessable, not chosen or enumerable by a
    consumer, and useless on its own — every request made through it crosses
    the same enforcement chain and audit as a network request would.
    """
    if not callable(app):
        raise TypeError("a contract channel needs the component's ASGI application")
    handle = f"rbc-{secrets.token_urlsafe(16)}"
...
def call_contract(
    ...
    if channel is None:
        raise ContractViolation("the records contract channel is closed")
...
def revoke_on_death(owner: Any, handle: str) -> None:
    ...
    object lives the contract is executable, and once it is gone the table
    holds nothing — so the provider never keeps an application alive on its
    own account. ``atexit`` is off: a half-torn-down interpreter has nothing
    to revoke.
    """
...
def asgi_transport(app: Any) -> ContractTransport:
    ...
    published path — a consumer receives a channel handle from
    :func:`open_channel` instead, so nothing it holds references the
    application.
    """
